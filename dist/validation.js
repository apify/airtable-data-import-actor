...
export const validateInput = (input) => {
    ...
    if (!base)
        throw new Error('Input "base" (Airtable base ID) is required.');
    if (!table)
        throw new Error('Input "table" (Airtable table name) is required.');
    if (!datasetId)
        throw new Error('Input "datasetId" is required.');
    if (!Array.isArray(cleanedMappings) || cleanedMappings.length === 0) {
        throw new Error('Input "dataMappings" must be a non-empty array.');
    }
};
export const ensureTable = async (airtable, baseId, tableNameOrId, operation) => {
    ...
    if (!table) {
        throw new Error(`Table "${tableNameOrId}" was not found in base "${baseId}". ` +
            `If you intend to create it, use operation "create" and implement createTableIfSupported().`);
    }
    return table;
};
export const ensureFieldsExist = async (_airtable, _baseId, table, dataMappings) => {
    ...
    for (const mapping of dataMappings) {
        ...
        if (!exists) {
            if (targetType === 'existing') {
                throw new Error(`Mapping expects existing Airtable field "${target}" in table "${table.name}", ` +
    ...
    if (!missingFields.length)
        return;
    throw new Error(`The following fields are missing in table "${table.name}": ` +
